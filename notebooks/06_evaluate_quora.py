...
IDX_DIR = Path("data/indexes/quora")
...
queries_df = pd.read_csv("data/processed/quora/queries_processed.csv")
qrels      = load_qrels("data/raw/quora/qrels.csv")

queries_with_qrels = set(qrels.keys())
queries_df = queries_df[queries_df["query_id"].astype(str).isin(queries_with_qrels)]
queries_with_qrels = set(qrels.keys())
queries_df = queries_df[queries_df["query_id"].astype(str).isin(queries_with_qrels)]
print(f"📊 Evaluating on {len(queries_df)} queries from Quora\n")

TOPK = 10

def fn_tfidf(tokens):
    return retrieve_tfidf(tokens, top_k=TOPK, dataset="quora")

def fn_bm25(tokens):
    return retrieve_bm25(tokens, top_k=TOPK, k1=1.5, b=0.75, dataset="quora")

def fn_embed(tokens):
    return retrieve_embedding(" ".join(tokens), top_k=TOPK, dataset="quora")

def fn_serial(tokens):
    return retrieve_hybrid_serial(" ".join(tokens), tokens, top_k=TOPK, dataset="quora")

def fn_parallel_rrf(tokens):
...
def fn_rag(tokens):
    q_str = " ".join(tokens)
    ans = rag_answer(q_str, tokens, dataset="quora", top_k=TOPK)
...
def fn_bm25_refined(tokens):
    ...
    return retrieve_bm25(new_tokens, top_k=TOPK, k1=1.5, b=0.75, dataset="quora")
...
models = {
    ...
    "Hybrid Parallel (RRF)": fn_parallel_rrf,
...
print("\n" + "═"*60)
print("  📊 EVALUATION SUMMARY — Quora")
print("═"*60)
print(summary.to_string(index=False))
print("═"*60)

summary.to_csv("data/processed/quora/evaluation_results.csv", index=False)
print("\n✅ Saved → data/processed/quora/evaluation_results.csv")